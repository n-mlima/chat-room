...
const Profile = ({ setActiveRoute, profile, setSocket, setDescriptionRoom, setRoomName }) => {
  ...
  const addRoom = async (e) => {
    ...
    try {
      await axios.post(`${API}/room`, listRoom);
      setRoom((prev) => [...prev, listRoom]);
  ...
  const handleDelete = async (id, ev) => {
    ...
    try {
      ...
      setRoom((prev) => prev.filter((item) => item.id !== id));
    } catch (error) {
  ...
  return (
    <>
      <Modal className={"modal"} isOpen={modalIsOpen} onRequestClose={closeModal} contentLabel="Adicionar Sala">
        ...
        <form className={"form"} onSubmit={addRoom}>
          <div className="div-form">
            ...
            <div className="div-login-group">
              ...
              <input id="nameRoom" placeholder="ex.:Trabalho" onChange={(e) => setNameRoom(e.target.value)} value={nameRoom} required />
            </div>

            <div className="div-login-group">
              <label htmlFor="data">Criado em:</label>
              <input id="data" defaultValue={data} />
            </div>

            <div className="div-login-group">
              <label htmlFor="description">Descrição:</label>
              <input id="description" placeholder="Descrição da sala" onChange={(e) => setDescription(e.target.value)} value={description} required />
            </div>

            <div className="div-login-group">
              <label htmlFor="author">Criado por:</label>
              <input id="author" defaultValue={author} disabled />
            </div>
            ...
            <div className="div-login-group">
              ...
              <input id="invite" defaultValue={invite} disabled />
            </div>
        ...
        <div className="container-cg">
          ...
          <ul className="profile-ul">
            <li onClick={openModal}>
              <MdOutlineAddComment /> Criar uma sala
            </li>
            <li>
              <GoPasskeyFill /> Configurações de conta
            </li>
          </ul>
          <button className="logout-btt" onClick={logout}>
            Sair
          </button>
        </div>
          ...
          <div className="subcontainer-room">
            {confirmInvite && (
              <div className="code-notification">
                ...
                <button className="btt-privacy" onClick={() => validInvite(roomAtual)}>
                  Ok
                </button>
              </div>
            )}
